...
public class ExcelUtil<T> {
    ...
    public ExcelUtil(FieldHandler handler) {
     ...
     */
    public void generateModel(Object instance, String filedName, String value) throws Exception {
        // 获取这个类中和这个字段相关的信息
    ...
    public void writeField(Object instance, Field field, String value) throws Exception {
        // 获取字段名，将字段名的首字符大写，方便构造get，set方法
        String originFieldName = field.getName();
        String fieldName = originFieldName.substring(0, 1).toUpperCase() + originFieldName.substring(1);
        String fieldType = field.getGenericType().toString();

        boolean flag=false;
        //字段的Display注解ConstValue有指定值，则将字段值写死
        List<ClassMetadata> globalFieldMappings = GlobalMetadata.getGlobalFieldMappings();
        for (ClassMetadata metadata : globalFieldMappings) {
            if (originFieldName.equals(metadata.getFieldName())&&
                instance.getClass().getName().equals(metadata.getBelongClassName())&&
                StringUtils.isNoneBlank(metadata.getConstValue())){
                value=metadata.getConstValue();
                flag=true;
            }
        }
        if (StringUtils.isBlank(value)) return;//单元格内容为空

        //执行属性处理器的抽象逻辑,如果Display.constValue()有值则不执行此逻辑
        if (!flag){
            value = this.handler.intercept(field, value);
        }
        ...
        switch (fieldType) {
            ...
            case "class java.math.BigDecimal": {
                Method m = instance.getClass().getMethod("set" + fieldName, BigDecimal.class);
            ...
            case "class java.util.Date": {
                Method m = instance.getClass().getMethod("set" + fieldName, Date.class);
